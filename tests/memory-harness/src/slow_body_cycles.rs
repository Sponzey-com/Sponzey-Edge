...
pub const SLOW_BODY_CYCLE_COUNT: u32 = 5;
pub const SLOW_BODY_CONNECTIONS: u64 = 128;
pub const SLOW_BODY_MINIMUM_PAYLOAD_BYTES: u64 = 128 * 32_768;
pub const SLOW_BODY_RSS_CEILING_BYTES: u64 = 512 * 1024 * 1024;
pub const SLOW_BODY_PLATEAU_FLOOR_BYTES: u64 = 16 * 1024 * 1024;

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct SlowBodyCycleObservation {
    pub cycle_index: u32,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct SlowBodyCycleInput {
    pub observations: Vec<SlowBodyCycleObservation>,
}

impl SlowBodyCycleInput {
    pub fn from_json(bytes: &[u8]) -> Result<Self, HarnessError> {
        serde_json::from_slice(bytes)
            .map_err(|_| HarnessError::new("slow body cycle input is invalid"))
    }
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct SlowBodyCycleResult {
    pub cycle_index: u32,
    pub peak_rss_bytes: u64,
    pub cooldown_rss_bytes: u64,
    pub held_payload_bytes: u64,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct SlowBodyCycleReport {
    ...
    pub cycles: Vec<SlowBodyCycleResult>,
}

impl SlowBodyCycleReport {
    pub fn validate(&self) -> Result<(), HarnessError> {
        if self.schema_version != 1
            || self.profile_id != "phase011-slow-body-5cycle-v1"
            ...
            || self.cycle_count != SLOW_BODY_CYCLE_COUNT
            || self.expected_per_cycle != SLOW_BODY_CONNECTIONS
            || self.cycles.len() != SLOW_BODY_CYCLE_COUNT as usize
            || !self.plateau_passed
            || self.correctness_failures != 0
            || self.cleanup_failures != 0
            || self.rss_ceiling_bytes != SLOW_BODY_RSS_CEILING_BYTES
        {
            return Err(HarnessError::new(
                "slow body cycle report header is invalid",
            ));
        }
        ...
        if self.plateau_tolerance_bytes != expected_tolerance
            || self.last_cooldown_median_rss_bytes
                > self
                    ...
                    .ok_or_else(|| HarnessError::new("slow body plateau overflows"))?
        {
            return Err(HarnessError::new("slow body cycle plateau is invalid"));
        }
        for (position, cycle) in self.cycles.iter().enumerate() {
            if cycle.cycle_index != position as u32 + 1
                || cycle.peak_rss_bytes == 0
                || cycle.peak_rss_bytes > SLOW_BODY_RSS_CEILING_BYTES
                || cycle.cooldown_rss_bytes == 0
                || cycle.cooldown_rss_bytes > cycle.peak_rss_bytes
                || cycle.held_payload_bytes < SLOW_BODY_MINIMUM_PAYLOAD_BYTES
            {
                return Err(HarnessError::new("slow body cycle result is invalid"));
            }
        }
        ...
        {
            return Err(HarnessError::new(
                "slow body cycle cooldown summary is invalid",
            ));
        }
        Ok(())
    }
    ...
    pub fn to_canonical_json(&self) -> Result<String, HarnessError> {
        ...
        let mut encoded = serde_json::to_string_pretty(self)
            .map_err(|_| HarnessError::new("slow body cycle encoding failed"))?;
        encoded.push('\n');
        Ok(encoded)
    }
    ...
    pub fn from_canonical_json(bytes: &[u8]) -> Result<Self, HarnessError> {
        let report: Self = serde_json::from_slice(bytes)
            .map_err(|_| HarnessError::new("slow body cycle decoding failed"))?;
        report.validate()?;
        if report.to_canonical_json()?.as_bytes() != bytes {
            return Err(HarnessError::new("slow body cycle report is not canonical"));
        }
        Ok(report)
    }
}

pub fn evaluate_slow_body_cycles(
    observations: Vec<SlowBodyCycleObservation>,
) -> Result<SlowBodyCycleReport, HarnessError> {
    if observations.len() != SLOW_BODY_CYCLE_COUNT as usize {
        return Err(HarnessError::new(
            "slow body profile requires exactly five cycles",
        ));
    }
    ...
    for (position, observation) in observations.iter().enumerate() {
        if observation.cycle_index != position as u32 + 1
            ...
            || observation.expected != SLOW_BODY_CONNECTIONS
            || observation.succeeded != SLOW_BODY_CONNECTIONS
            || observation.failed != 0
            || observation.held_payload_bytes < SLOW_BODY_MINIMUM_PAYLOAD_BYTES
            || observation.peak_rss_bytes == 0
            || observation.peak_rss_bytes > SLOW_BODY_RSS_CEILING_BYTES
        ...
        {
            return Err(HarnessError::new("slow body cycle observation is invalid"));
        }
        cycles.push(SlowBodyCycleResult {
    ...
    {
        return Err(HarnessError::new(
            "slow body cooldown plateau threshold exceeded",
        ));
    }
    let report = SlowBodyCycleReport {
        schema_version: 1,
        profile_id: "phase011-slow-body-5cycle-v1".to_string(),
        ...
        cycle_count: SLOW_BODY_CYCLE_COUNT,
        expected_per_cycle: SLOW_BODY_CONNECTIONS,
        ...
        cleanup_failures: 0,
        rss_ceiling_bytes: SLOW_BODY_RSS_CEILING_BYTES,
        cycles,
    };
    report.validate()?;
    Ok(report)
...
fn plateau_tolerance(first: u64) -> Result<u64, HarnessError> {
    if first == 0 {
        return Err(HarnessError::new("slow body plateau baseline is invalid"));
    }
    Ok(SLOW_BODY_PLATEAU_FLOOR_BYTES.max(first / 10))
}

fn median_of_pair(left: u64, right: u64) -> Result<u64, HarnessError> {
    left.checked_add(right)
        .map(|sum| sum / 2)
        .ok_or_else(|| HarnessError::new("slow body cooldown median overflows"))
}
