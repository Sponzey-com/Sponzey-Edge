...
use edge_memory_harness::websocket_cycles::{
    evaluate_websocket_cycles, WebSocketCycleInput, WebSocketCycleReport,
};
use edge_memory_harness::HarnessError;

fn main() {
    if let Err(error) = run(env::args().skip(1).collect()) {
        eprintln!("WebSocket cycles error: {error}");
        std::process::exit(1);
    }
}
...
fn run(arguments: Vec<String>) -> Result<(), HarnessError> {
    ...
    match command.as_str() {
        "collect" => {
            ...
            let input = WebSocketCycleInput::from_json(&read_regular(Path::new(required(
                &options, "--input",
            )?))?)?;
            let report = evaluate_websocket_cycles(input.observations)?;
            ...
            println!(
                "WebSocket cycles collected cycles={} first_median={} last_median={} digest={}",
                report.cycle_count,
        ...
        "validate" => {
            ...
            if sha256_hex(&report_bytes) != digest {
                return Err(HarnessError::new("WebSocket cycle digest mismatch"));
            }
            let report = WebSocketCycleReport::from_canonical_json(&report_bytes)?;
            if report.build_identity != required(&options, "--build-identity")? {
                return Err(HarnessError::new(
                    "WebSocket cycle source identity mismatch",
                ));
            }
            println!(
                "WebSocket cycles validated cycles={} plateau=passed digest={}",
                report.cycle_count, digest
            );
        }
        _ => return Err(HarnessError::new("unknown WebSocket cycle command")),
    }
    Ok(())
}
...
fn parse(arguments: Vec<String>) -> Result<(String, BTreeMap<String, String>), HarnessError> {
    ...
    let command = values
        .next()
        .ok_or_else(|| HarnessError::new("WebSocket cycle command is required"))?;
    let mut options = BTreeMap::new();
    while let Some(key) = values.next() {
        if !key.starts_with("--") || options.contains_key(&key) {
            return Err(HarnessError::new("WebSocket cycle option is invalid"));
        }
        let value = values
            .next()
            .filter(|value| !value.is_empty())
            .ok_or_else(|| HarnessError::new("WebSocket cycle option value is missing"))?;
...
fn exact_keys(options: &BTreeMap<String, String>, expected: &[&str]) -> Result<(), HarnessError> {
    if options.len() != expected.len() || expected.iter().any(|key| !options.contains_key(*key)) {
        return Err(HarnessError::new("WebSocket cycle option set is invalid"));
    }
    Ok(())
}
...
fn required<'a>(options: &'a BTreeMap<String, String>, key: &str) -> Result<&'a str, HarnessError> {
    options
        .get(key)
        .map(String::as_str)
        .ok_or_else(|| HarnessError::new("WebSocket cycle option is missing"))
}

fn read_regular(path: &Path) -> Result<Vec<u8>, HarnessError> {
    let metadata = fs::symlink_metadata(path)
        .map_err(|_| HarnessError::new("WebSocket cycle file is unavailable"))?;
    if metadata.file_type().is_symlink() || !metadata.is_file() {
        return Err(HarnessError::new(
            "WebSocket cycle file must be physical and regular",
        ));
    }
    fs::read(path).map_err(|_| HarnessError::new("WebSocket cycle file cannot be read"))
}

fn parse_digest(bytes: &[u8]) -> Result<String, HarnessError> {
    let text = std::str::from_utf8(bytes)
        .map_err(|_| HarnessError::new("WebSocket cycle digest is not UTF-8"))?;
    let digest = text
        .strip_suffix('\n')
        .filter(|value| !value.contains('\n'))
        .ok_or_else(|| HarnessError::new("WebSocket cycle digest is not canonical"))?;
    Ok(digest.to_string())
}
