use edge_memory_harness::websocket_cycles::{
    evaluate_websocket_cycles, WebSocketCycleObservation, WebSocketCycleReport,
    WEBSOCKET_CYCLE_COUNT,
};
...
const PROCESS: &str = "macos-lstart:websocket-process";

#[test]
fn exactly_five_clean_cycles_pass_median_plateau_and_roundtrip() {
    let report = evaluate_websocket_cycles(cycles([11, 13, 18, 15, 17])).unwrap();

    assert_eq!(report.cycle_count, WEBSOCKET_CYCLE_COUNT);
    ...
    assert_eq!(
        WebSocketCycleReport::from_canonical_json(canonical.as_bytes()).unwrap(),
        report
    );
}
...
fn count_order_identity_correctness_payload_and_cleanup_fail_closed() {
    assert!(evaluate_websocket_cycles(cycles([1, 1, 1, 1])).is_err());

    let mut invalid = cycles([1, 1, 1, 1, 1]);
    invalid[2].cycle_index = 4;
    assert!(evaluate_websocket_cycles(invalid).is_err());

    let mut invalid = cycles([1, 1, 1, 1, 1]);
    invalid[2].process_start_identity = "macos-lstart:other".to_string();
    assert!(evaluate_websocket_cycles(invalid).is_err());

    for mutate in ["upgraded", "echoed", "held", "released", "failed"] {
        let mut invalid = cycles([1, 1, 1, 1, 1]);
        match mutate {
            "upgraded" => invalid[2].upgraded = 127,
            "echoed" => invalid[2].echoed = 127,
            "held" => invalid[2].held = 127,
            "released" => invalid[2].released = 127,
            "failed" => invalid[2].failed = 1,
            _ => unreachable!(),
        }
        assert!(evaluate_websocket_cycles(invalid).is_err());
    }
    ...
    let mut invalid = cycles([1, 1, 1, 1, 1]);
    invalid[4].cleanup_payload_bytes = 1;
    assert!(evaluate_websocket_cycles(invalid).is_err());
}
...
fn ceiling_and_plateau_threshold_plus_one_fail() {
    let mut invalid = cycles([1, 1, 1, 1, 1]);
    invalid[1].peak_rss_bytes = 384 * 1024 * 1024 + 1;
    assert!(evaluate_websocket_cycles(invalid).is_err());

    let baseline = 10_000_000;
    let tolerance = 16 * 1024 * 1024;
    assert!(evaluate_websocket_cycles(cycles([
        baseline,
        baseline,
        baseline,
...
fn cycles<const N: usize>(cooldowns: [u64; N]) -> Vec<WebSocketCycleObservation> {
    cooldowns
        .into_iter()
        .enumerate()
...
fn observation(index: u32, peak: u64, cooldown: u64) -> WebSocketCycleObservation {
    WebSocketCycleObservation {
        cycle_index: index,
